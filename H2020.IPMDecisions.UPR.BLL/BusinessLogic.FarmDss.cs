...
namespace H2020.IPMDecisions.UPR.BLL
{
    public partial class BusinessLogic : IBusinessLogic
    {
        ...
        public async Task<GenericResponse<IDictionary<string, object>>> AddListOfLinkDssToFarm(IEnumerable<FarmDssForCreationDto> farmDssDto, HttpContext httpContext, string mediaType)
        {
            try
            {
                var userId = Guid.Parse(httpContext.Items["userId"].ToString());

                Farm existingFarm = await this.dataService
                    .Farms
                    .FindByConditionAsync(
                        f => f.UserFarms.Any(uf =>
                            uf.UserId.Equals(userId))
                        && f.Fields.Any(fi =>
                            fi.FieldCrop.FieldCropPests.Any(fcp =>
                                fcp.CropPest.CropPestDsses.Any(cpd =>
                                    cpd.DssExecutionType.ToLower() == "link")))
                        , true);

                if (existingFarm == null)
                {
                    var newFarm = new FarmForCreationDto()
                    {
                        Name = "Autogenerated_ExternalDss_Farm",
                        Location = new CustomPointLocation(0, 0, 4326)
                    };
                    var response = await LinkNewFarmToUserProfile(newFarm, userId, mediaType);
                    var farmId = Guid.Parse(response.Result["Id"].ToString());
                    existingFarm = await this.dataService
                        .Farms.FindByIdAsync(farmId, true);
                }
                httpContext.Items.Add("farm", existingFarm);
                return await AddListOfFarmDss(farmDssDto, httpContext, mediaType);
            }
            catch (Exception ex)
            {
                logger.LogError(string.Format("Error in BLL - AddListOfLinkDssToFarm. {0}", ex.Message), ex);
                String innerMessage = (ex.InnerException != null) ? ex.InnerException.Message : "";
                return GenericResponseBuilder.NoSuccess<IDictionary<string, object>>(null, $"{ex.Message} InnerException: {innerMessage}");
            }
        }
