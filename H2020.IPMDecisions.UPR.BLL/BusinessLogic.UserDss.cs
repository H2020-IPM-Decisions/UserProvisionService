...
namespace H2020.IPMDecisions.UPR.BLL
{
    public partial class BusinessLogic : IBusinessLogic
    {
        ...
        public async Task<GenericResponse<IEnumerable<IGrouping<string, DssResultFromDatabase>>>> GetAllDssResults(Guid userId)
        {
            try
            {
                ...
                return GenericResponseBuilder.Success<IEnumerable<IGrouping<string, DssResultFromDatabase>>>(dssResultGroup);
            }
            catch (Exception ex)
            {
                logger.LogError(string.Format("Error in BLL - GetAllUserFieldCropPestDss. {0}", ex.Message), ex);
                String innerMessage = (ex.InnerException != null) ? ex.InnerException.Message : "";
                return GenericResponseBuilder.NoSuccess<IEnumerable<IGrouping<string, DssResultFromDatabase>>>(null, $"{ex.Message} InnerException: {innerMessage}");
            }
        }
    }
}